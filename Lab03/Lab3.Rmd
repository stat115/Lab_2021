...
```{r clustering}
expr.dat <- t(edata)
#expr.dat <- t(log2(tpm+1))

### PCA: before batch effect
PCA1 <- prcomp(expr.dat)$x
df1 <- cbind.data.frame(PCA1,batch = factor(pheno$batch),
                        cancer = factor(pheno$cancer))
###plot
ggplot(df1, aes(x = PC1, y = PC2, shape = cancer, color = batch)) + 
  geom_point(size=3) + 
  labs(title="Before Batch effect removal")+ 
  theme_bw()
temp <- expr.dat
rownames(temp) <- paste(rownames(expr.dat), as.character(pheno$batch), sep = "_")
dd <- dist(scale(temp), method = "euclidean")
hc <- hclust(dd, method = "ward.D2")
plot(hc, hang = -1, cex = 0.6, main = "Before batch effect removal")
```
...
```{r combat-clusters}
expr.combat <- t(combat_edata)

###PCA: after batch effect removal
PCA2 <- prcomp(expr.combat)$x
df2 <- cbind.data.frame(PCA2,
                        batch = factor(pheno$batch),
                        tissue = factor(pheno$cancer))
###plot
ggplot(df2, aes(x = PC1, y = PC2, shape = tissue, color = batch)) +
  geom_point(size=3) +
